...
<?php if ($field->files()->count() > 0): ?>
  <div class="input input-with-items">
    <?php foreach ($field->files() as $file): ?>
        <div id="<?= $field->itemId($file) ?>" class="item item-condensed item-with-image [ selector-item js-selector-item ]" data-file="<?= $file->filename() ?>" <?= r($field->isInValue($file), 'data-checked="true"') ?> >
            ...
            <nav class="item-options">
                <ul class="nav nav-bar">
                    ...
                    <li>
                        <a class="btn btn-with-icon [ selector-checkbox js-selector-checkbox ]" href="#" title="<?= l('pages.show.subpages.add') ?>">
                          <i class="icon icon-left fa fa-circle-o"></i>
                          <span><?= l('pages.show.subpages.add') ?></span>
                        </a>
                    </li>
                </ul>
            </nav>
